...
if __name__ == '__main__':
    ...
    # board_setup_initial = ['rnbqkbnr', 'pppppppp', '', '', '', '', 'PPPPPPPP', 'RNBQKBNR']
    ...
    for row, each in enumerate(board_setup):
        for col, piece in enumerate(each):
            ...
            if piece in list_pieces:
                piece_positionmap["%s%d" % (piece,col)] = [pose.position.x, pose.position.y, pose.position.z-0.93]
                poses["%s%d" % (piece,col)] = pose
                piece_names.append("%s%d" % (piece,col))
